...
symbols_7 = symbols_5 + ['600036', '601166']
...
def backtest_full_grid(init_cap, s_date, e_date, data, symbols, common_dates, step=0.05):
    ...
    # Grid value is 20% of the target per stock
    grid_val = target * 0.2
...
data_7, dates_7 = load_data(symbols_7)

max_date = dates_7[-1]
horizons = ['1Y', '2Y', '3Y', '5Y']
init_cap = 30000.0

print("| 时间 | 5大行-平准(收益) | 5大行-网格(收益) | 7家行-平准(收益) | 7家行-网格(收益) |")
print("|---|---|---|---|---|")
for h in horizons:
    ...
    r7_p, m7_p = backtest_periodic(init_cap, sd, max_date, data_7, symbols_7, dates_7)
    r7_g, m7_g = backtest_full_grid(init_cap, sd, max_date, data_7, symbols_7, dates_7)
    print(f"| {h} | {r5_p*100:.2f}% | {r5_g*100:.2f}% | {r7_p*100:.2f}% | {r7_g*100:.2f}% |")

print("\n| 时间 | 5大行-平准(回撤) | 5大行-网格(回撤) | 7家行-平准(回撤) | 7家行-网格(回撤) |")
print("|---|---|---|---|---|")
for h in horizons:
    sd = max_date - relativedelta(years=int(h[0]))
    r5_p, m5_p = backtest_periodic(init_cap, sd, max_date, data_5, symbols_5, dates_5)
    ...
    r7_p, m7_p = backtest_periodic(init_cap, sd, max_date, data_7, symbols_7, dates_7)
    r7_g, m7_g = backtest_full_grid(init_cap, sd, max_date, data_7, symbols_7, dates_7)
    print(f"| {h} | {m5_p*100:.2f}% | {m5_g*100:.2f}% | {m7_p*100:.2f}% | {m7_g*100:.2f}% |")
