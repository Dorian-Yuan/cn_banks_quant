...
BARK_KEY = "XbLpUuV9b4wh2ZExePySME"
...
def send_bark(title, body):
    try:
        ...
        url = f"{BARK_API}/{BARK_KEY}/mc_dca?group=quant&title={t}&body={b}"
        urllib.request.urlopen(url, timeout=5)
    except Exception:
        pass


def compute_injection_dates(date_strs):
    injection_dates = set()
    ...
    for ds in date_strs:
        ...
        if month in [1, 4, 7, 10]:
            if month != current_quarter_month:
                if current_best:
                    injection_dates.add(current_best)
                current_quarter_month = month
                current_best = None
            if day <= 6:
                current_best = ds
        else:
            if current_quarter_month is not None:
                if current_best:
                    injection_dates.add(current_best)
                current_quarter_month = None
                current_best = None
...
def do_rebalance(current_banks, fast, idx, portfolio, cash, base_p, lots, init_shares,
                 open_position, current_ordinal, config):
    total_value = cash
    stock_prices = {}
...
def run_backtest_with_dca(banks_init, fast, date_strs, n_days,
                          entry_prices, hold_days, injection_dates,
                          psbc_ordinal=PSBC_ORDINAL):
    N = len(banks_init)
    ...
    if init_total_cost > total_capital:
        total_capital = init_total_cost
        cash = total_capital
    ...
    for idx in range(target_end_idx):
        ...
        is_injection_day = date_str in injection_dates
        is_month_end = is_period_end(date_str, next_date_str, 'month')

        if is_injection_day:
            cash += INJECTION_AMOUNT
            total_capital += INJECTION_AMOUNT
            injection_count += 1
            cash = do_rebalance(current_banks, fast, idx, portfolio, cash, base_p,
                                lots, init_shares, open_position, current_ordinal, config)
            rebalanced_today = True

        if is_month_end and not rebalanced_today:
            cash = do_rebalance(current_banks, fast, idx, portfolio, cash, base_p,
                                lots, init_shares, open_position, current_ordinal, config)
    ...
    return total_return, max_drawdown * 100, total_capital, injection_count, final_value


def main():
    send_bark("任务开始", "开始运行季度定投蒙特卡洛模拟: 卖1.2买0.5网格+单次30%+月度再平准+3w本金+每季加1万, 2500样本")

    print("=" * 90)
    print("  蒙特卡洛网格策略盈利概率分析 (含季度定投)")
    print("  策略: 卖1.2%买0.5% + 单次30% + 月度再平准 + 3万本金 + 每季加1万")
    print("=" * 90)
    print(f"\n  参数:")
    ...
    print(f"    季度定投: 每季末月(1/4/7/10)6号或之前最近交易日加{INJECTION_AMOUNT}元")
    ...
    injection_dates = compute_injection_dates(date_strs_all)
    print(f"\n  数据范围: {full_dates[0].strftime('%Y-%m-%d')} ~ {full_dates[-1].strftime('%Y-%m-%d')}")
    print(f"  交易日数: {len(full_dates)}")
    print(f"  季度注入日数: {len(injection_dates)}")
    sample_injection = sorted([d for d in injection_dates if '2024' in d])[:8]
    print(f"  注入日示例(2024): {', '.join(sample_injection)}")
    ...
    for i in range(N_SAMPLES):
        ...
        for label, h_days in HOLD_PERIODS.items():
            ...
            sub_injection = set()
            for ds in date_strs:
                if ds in injection_dates:
                    sub_injection.add(ds)
            ...
            ret, max_dd, total_cap, inj_count, final_val = run_backtest_with_dca(
                banks_init, fast, date_strs, n_days,
                entry_prices, h_days, sub_injection, PSBC_ORDINAL
            )
    ...
    print(f"{'=' * 100}")
    print(f"{'持有期':>10} {'盈利概率':>10} {'中位收益率':>12} {'中位回撤':>10} {'平均注入':>10} {'平均总投入':>12} {'样本数':>8}")
    print("-" * 100)
    ...
    for label in hold_labels:
        ...
        injections = [r['injection_count'] for r in results]
        total_caps = [r['total_capital'] for r in results]
        med_ret = np.median(returns)
        med_dd = np.median(drawdowns)
        avg_inj = np.mean(injections)
        avg_cap = np.mean(total_caps)
        print(f"{label:>10} {prob:>9.1f}% {med_ret:>+11.2f}% {med_dd:>9.2f}% {avg_inj:>9.1f}次 {avg_cap:>11.0f}元 {n:>8}")
    ...
    for label in hold_labels:
        ...
        if results:
            prob = sum(1 for r in results if r['return'] > 0) / len(results) * 100
            med_ret = np.median([r['return'] for r in results])
            summary_parts.append(f"{label}:{prob:.0f}%/+{med_ret:.1f}%")
    summary = ", ".join(summary_parts)
    send_bark("任务完成", f"季度定投蒙特卡洛模拟完成({elapsed:.0f}秒). {summary}")
